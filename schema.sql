...
using (
  published=true and exists(
    ...
    where i.id=monthly_reports.investor_id and i.auth_user_id=auth.uid() and i.active=true
  )
);

-- MEMBUAT OWNER PERTAMA
-- 1. Supabase Dashboard > Authentication > Users > Add user.
--    Email disarankan: owner@mulia-investor.local
-- 2. Salin UUID user tersebut.
-- 3. Jalankan contoh berikut setelah mengganti UUID dan username:
--
-- insert into public.profiles(auth_user_id,username,full_name,role)
-- values ('UUID-USER-OWNER','owner','Abdul Aziz Efendi','owner');
